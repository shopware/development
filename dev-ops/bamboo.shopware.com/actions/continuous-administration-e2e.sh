...
docker exec -u __USERKEY__ __CYPRESS_ID__ npm clean-install --prefix /e2e-Administration
docker exec -u __USERKEY__ __CYPRESS_ID__ npx cypress run --project /e2e-Administration --browser chrome --config baseUrl=__APP_URL__ || echo Failures: $?
